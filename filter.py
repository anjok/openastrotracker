...
class MPU:
    ...
    def compFilter(self):
        ...
        #
        self.x = math.degrees(math.atan2(self.ax, dist(self.ay,self.az)))
        self.y = math.degrees(math.atan2(self.ay, dist(self.ax,self.az))) 

        # Comp filter
        self.roll = ((self.tau)*(self.roll - self.gy*dt) + (1-self.tau)*(accRoll))
        self.pitch = ((self.tau)*(self.pitch + self.gx*dt) + (1-self.tau)*(accPitch))
        (x,y,z) = (4.5,13.8,0) # 6050
        (x,y,z) = (29.1,18.3,0) # 9250
        #(x,y,z) = (0,0,0)
        
        # Print data
        print(f"\r R: {self.roll-x:5.2f} P: {self.pitch-y:5.2f} Y: {self.yaw-z:5.2f} | x: {self.x-x:5.2f} y: {self.y-y:5.2f} | x: {self.ax:5.2f} y: {self.ay:5.2f} z: {self.az:5.2f}", end = '')
